class MissionariesAndCannibals {
  ...
  List<CurrentState> findSolutionByBFS() {/// 3,3,1
    ///                                      2,3,0 3,2,0
    ///
    final runtime = Stopwatch();
    runtime.start();
  ...
  List<CurrentState> Children(CurrentState state) {
    ...
    if (state.boatAtRight ? state.monster >= 1 : state.monster < 3)
      list.add(CurrentState(
          state.human, state.monster + (1 * sgin), !state.boatAtRight, state));
...
bool contain(CurrentState state, List list) {
  for (var x in list) {
    if (x.human == state.human &&
        x.monster == state.monster &&
        x.boatAtRight == state.boatAtRight) return true;
  }
  return false;
}
...
class CurrentState {
  ...
  bool isValid() {/// 3,3,1
    ///
    return (human >= monster || human == 0) &&
        (3 - human >= 3 - monster || human == 3);
  }
}
